...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ onBack, vaultName }) => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: `Vault AI secure environment initialized. Access granted to vault: ${vaultName}. How can I assist you today?`,
      timestamp: new Date()
    }
  ]);
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const errorResponse: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: "I'm sorry, an error occurred while connecting to the database.",
        timestamp: new Date()
      };
      ...
      <div className="flex-1 overflow-y-auto px-4 custom-scrollbar space-y-6 pb-4">
        <AnimatePresence>
          {messages.map((msg) => (
            ...
            >
              <div
                className={`max-w-[80%] md:max-w-[70%] p-4 rounded-2xl backdrop-blur-md border ${msg.role === 'user'
          ...
          {isLoading && (
            ...
            >
              <div className="p-4 rounded-2xl backdrop-blur-md border bg-white/5 border-white/10 shadow-lg flex items-center gap-1.5 h-[52px]">
